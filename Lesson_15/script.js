...
// const elements = {
//   list: document.querySelector(".products-container"),
// };
...
//         price,
//         description,
//       }) => `<li class="product-card" data-card-id="${id}" >
//         <img class="product-img" src="${img}" alt="${name}" />
//         <h2 class="product-title">${name}</h2>
//         <p class="product-description">${description}</p>
//         <p class="product-price">${price} грн.</p>
//         <button class="product-add-btn" type="button">Add to basket</button>
//       </li>`
//     )
//     .join("");
// }
...
// elements.list.addEventListener("click", handlerAddBtn);

// function handlerAddBtn({ target }) {
//   if (target.classList.contains("product-add-btn")) {
//     const card = target.closest(".product-card");
//     addToLS(data.find(({ id }) => id === +card.dataset.cardId));
//   }
// }
...
//   const item = basket.find(({ id }) => id === element.id);
//   if (item) {
//     item.qty++;
...
//   return s.split(", ").reduce((acc, el, idx, arr) => {
//     if (idx === arr.length - 1) {
//       const size = arr.length;
//       return (acc / ((size * arr[0].length * (size - 1)) / 2)) * 100;
//     } else {
//       arr
//         .slice(idx + 1)
//         .forEach((elInner) =>
//           elInner
//             .split("")
//             .forEach((char, charIdx) => (acc += +(char === el.charAt(charIdx))))
//         );
//       return acc;
//     }
//   }, 0);
...
function findScreenHeight(width, ratio, [w, h] = ratio.split(":")) {
